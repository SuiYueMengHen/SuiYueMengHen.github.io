...
describe('post utilities', () => {
  ...
  it('never includes collection chapters in categories',()=>{const posts=[makePost('loose','2026-01-01'),makePost('chapter','2026-01-02',{collection:'book',collectionOrder:1})];expect(categoryPosts(posts,'设计').map(item=>item.id)).toEqual(['loose'])});
